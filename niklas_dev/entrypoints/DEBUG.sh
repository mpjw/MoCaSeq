...
# - use GDC downloader to get bam, the use that
# - COPY results to encrypted drive mounted on done/ --> this step is a problem, removed for now
# - removed Check for matched BAM-files
# - allow path to bam (really? or did it just work and i did not change anything?)
...
echo "---- Downloading data from TCGA ----" | tee -a $name/results/QC/$name.report.txt
cohort_manifest=gdc_manifest.txt
cohort_samplesheet=gdc_sample_sheet.tsv
Rscript /opt/MoCaSeq/niklas_dev/TCGA_get_download_manifest_perSample.R ${name} ${cohort_samplesheet} ${cohort_manifest}

download_manifest="${name}/results/QC/${name}_download_manifest.txt"
echo ./gdc-client download -m ${download_manifest} -n 30 -t gdc-user-token.txt -d raw
./gdc-client download -m ${download_manifest} -n 30 -t gdc-user-token.txt -d raw
...
if [ -z $bam_normal ] && [ -z $bam_tumor ]; then
	...
	echo '---- Removing unpaired reads and sorting raw BAM ----' | tee -a $name/results/QC/$name.report.txt

	# remove unpaired reads and sort
	bam_tumor_basename=$(basename $bam_tumor | sed 's/.bam$/.sorted.filtered.bam/')
	bam_tumor_dirname=$(dirname $bam_tumor)
	bam_tumor_new="${bam_tumor_dirname}/${bam_tumor_basename}"
	samtools view -f 1 -hb $bam_tumor | samtools sort > ${bam_tumor_new}
	samtools index ${bam_tumor_new}
	bam_tumor=${bam_tumor_new}

	bam_normal_basename=$(basename $bam_normal | sed 's/.bam$/.sorted.filtered.bam/')
	bam_normal_dirname=$(dirname $bam_normal)
	bam_normal_new="${bam_normal_dirname}/${bam_normal_basename}"
	samtools view -f 1 -hb $bam_normal | samtools sort > ${bam_normal_new}
	samtools index ${bam_normal_new}
	bam_normal=${bam_normal_new}
...
# !TCGA CHANGE!
# delete BAM files folder
bamfolders=$(sed -n '1!p' "${name}/results/QC/${name}_download_manifest.txt" | cut -f1)
bamfolders=$(printf '%s\n' "$bamfolders" | sed 's/[^[:space:]]\{1,\}/raw\/&/g')
#rm -r $bamfolders &
...
if [ $runmode = "MS" ] && [ $BubbleTree = 'yes' ]; then
	...
	# !TCGA CHANGE!
	Rscript $repository_dir/all_RunBubbleTree.R $name "CNVKit" $species
fi
...
#!TCGA CHANGE!
# just move the finished samples to a subfolder
mv $name/ done/
...
# #!TCGA CHANGE!
# echo '---- Copying results to encrypted drive '$name' ----' | tee -a $name/results/QC/$name.report.txt
# #start copy in background, continue with next sample
# bash /opt/MoCaSeq/niklas_dev/TCGA_copy_results.sh ${name} &
# echo '---- Finished copy '$name' ----' | tee -a $name/results/QC/$name.report.txt
