...
if [ ${USERID:-0} -ne 0 ] && [ ${GRPID:-0} -ne 0 ]; then \
    ...
    chown --silent --no-dereference --recursive \
        ...
        /var/pipeline &&\
    exec gosu docker ${PACKAGE_DIR}/MoCaSeq/niklas_dev/entrypoints/MoCaSeq_InSilicoMix.sh "$@" \
;fi

exec ${PACKAGE_DIR}/MoCaSeq/niklas_dev/entrypoints/MoCaSeq_InSilicoMix.sh "$@"
