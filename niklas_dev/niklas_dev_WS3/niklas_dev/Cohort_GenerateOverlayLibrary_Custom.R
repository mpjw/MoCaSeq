...
fillOverlayMat = function(Samples,Paths,AberrationCutoff){
...
combineOverlayMat <- function(SummaryStat){
...
# some small changes:
# data as argument
# YTicks to specify the number of ticks on Y axis
PlotOverlay2 = function(OMatrix=OverlayMat, format="",Suffix="",Ylim="",Save_path="", YTicks=1){
  if (Save_path=="") { 
    PathPrefix=paste0(getwd(),"/") } else { 
      PathPrefix=paste0(Save_path,"/") }
  YcordsGain = OMatrix$Gain
  YcordsDel = OMatrix$Del
  Xcords = OMatrix$Start
  Xcords = c(0,Xcords,0)
  YcordsGain = c(YcordsGain[1],YcordsGain,rev(YcordsGain)[1])
  YcordsDel = c(YcordsDel[1],YcordsDel,rev(YcordsDel)[1])    
  if(SummaryStat=="Mean")
  {
    Ylim=as.numeric(Ylim)
    Ylim=c(-Ylim,Ylim)
    TickLabelPosition = seq(min(Ylim),max(Ylim),YTicks)
    TickLabel = seq(min(Ylim),max(Ylim),YTicks)
    Ylabelposition = 0
    Ylabeltext = "Mean log2 copynumber change"
  }
  if(SummaryStat=="Proportion")
  ...
  par(las=1)            
  plot(1,xlim=c(0,max(OMatrix$Stop)),ylim=Ylim,bty="n",xaxt="n",yaxt="n",xlab="",ylab="",type="n")
  polygon(Xcords,YcordsGain,col="#B22222",border="#B22222")
  polygon(Xcords,YcordsDel,col="#4682B4",border="#4682B4")
  segments(Len,min(Ylim),Len,max(Ylim),lty=3,col="grey40",lwd=0.9)
  mtext(side=1,line=1,at=LabelPos,Chromosomes)
...
PlotOverlay2Lines = function(OMatrix=OverlayMat, format="",Suffix="",Ylim="",Save_path="", YTicks=1){
  if (Save_path=="") { 
    PathPrefix=paste0(getwd(),"/") } else { 
      PathPrefix=paste0(Save_path,"/") }
  YcordsGain = OMatrix$Gain
  ...
  par(las=1)            
  plot(1,xlim=c(0,max(OMatrix$Stop)),ylim=Ylim,bty="n",xaxt="n",yaxt="n",xlab="",ylab="",type="n")
  lines(Xcords,YcordsGain,col="#B22222",border="#B22222")
  lines(Xcords,YcordsDel,col="#4682B4",border="#4682B4")
  segments(Len,min(Ylim),Len,max(Ylim),lty=3,col="grey40",lwd=0.9)
  mtext(side=1,line=1,at=LabelPos,Chromosomes)
  mtext(side=2,line=1,las=3,at=Ylabelposition,text=Ylabeltext)
  axis(side=2,line=-1.1,at=TickLabelPosition,labels=TickLabel)
...
# these functions were adapted, to take input and give output matrix instead of writing to global environment
SetOverlayMat2 <- function(Samples) {
  OverlayMatData = matrix(0,nrow=nrow(BinnedGenome),ncol=3+(length(Samples)*2))
  rownames(OverlayMatData) = rownames(BinnedGenome)
  colnames(OverlayMatData) = c("Chromosome","Start","Stop",paste(Samples,"Gain",sep=""),paste(Samples,"Del",sep=""))
  OverlayMatData = as.data.frame(OverlayMatData)
  OverlayMatData[,"Chromosome"] = BinnedGenome$chr
  OverlayMatData[,"Start"] = BinnedGenome$start
  OverlayMatData[,"Stop"] = BinnedGenome$end
  return(OverlayMatData)
}
fillOverlayMat2 = function(OverlayMatData,Samples,Paths,AberrationCutoff){
  for ( i in 1:length(Samples))
  {
    print(paste0("Getting data for ",Samples[i]))
    Segments = LoadSegmentData(Samples[i],Paths[i])
    ...
    for (segment in Segments$UnifiedPosition)
    {
      ...
      if(ActualPosition$Mean>0)
      {
        OverlayMatData[OverlayMatData[,"Chromosome"]==Chromosome & OverlayMatData[,"Start"]>=Start & OverlayMatData[,"Stop"]<=Stop,paste(Samples[i],"Gain",sep="")] = ActualPosition$Mean
      }
      if(ActualPosition$Mean<0)
      {
        OverlayMatData[OverlayMatData[,"Chromosome"]==Chromosome & OverlayMatData[,"Start"]>=Start & OverlayMatData[,"Stop"]<=Stop,paste(Samples[i],"Del",sep="")] = ActualPosition$Mean
      }
    }
  }
  OverlayMatData <<- OverlayMatData
}    
combineOverlayMat2 <- function(OverlayMatData,SummaryStat){
  ColumnsGainInfo = grep("Gain",colnames(OverlayMatData),value=T)
  ColumnsDelInfo = grep("Del",colnames(OverlayMatData),value=T)
  if(SummaryStat=="Mean")
  {
    OverlayMatData$Gain = apply(OverlayMatData[,ColumnsGainInfo,drop=F],1,mean)
    OverlayMatData$Del = apply(OverlayMatData[,ColumnsDelInfo,drop=F],1,mean)
  }
  if(SummaryStat=="Proportion")
  {
    AnnotationInfo = OverlayMatData[,c("Chromosome","Start","Stop")]
    CNVInfo = OverlayMatData[,c(ColumnsGainInfo,ColumnsDelInfo)]
    CNVInfo[CNVInfo!=0] = 1
    OverlayMatData = cbind(AnnotationInfo,CNVInfo)
    OverlayMatData$Gain = apply(OverlayMatData[,ColumnsGainInfo,drop=F],1,sum)/length(ColumnsGainInfo)
    OverlayMatData$Del = -1* apply(OverlayMatData[,ColumnsDelInfo,drop=F],1,sum)/length(ColumnsGainInfo)
  }    
  return(OverlayMatData)
}

combineOverlayMatForProp <- function(OverlayMatData,SummaryStat){
  ColumnsGainInfo = grep("Gain",colnames(OverlayMatData),value=T)
  ColumnsDelInfo = grep("Del",colnames(OverlayMatData),value=T)
  if(SummaryStat=="Mean")
  {
  ...
  if(SummaryStat=="Proportion")
  {
    ...
    OverlayMatData = cbind(AnnotationInfo,CNVInfo)
    OverlayMatData$Gain = apply(OverlayMatData[,ColumnsGainInfo,drop=F],1,sum)/length(ColumnsGainInfo)
    OverlayMatData$Del = apply(OverlayMatData[,ColumnsDelInfo,drop=F],1,sum)/length(ColumnsGainInfo)
  }    
  return(OverlayMatData)
}
...
ReformatOverlayMat2 = function(OverlayMatData){
  if("X" %in% names(ChromLength) & "Y" %in% names(ChromLength)){
    Chromosomes = names(ChromLength)[!names(ChromLength) %in% c("X","Y")]
    Chromosomes = c(sort(as.numeric(Chromosomes)),"X","Y")
  }
  ...
  ### Init vector with positions for chromosome endings on x-axis
  Len = max(OverlayMatData[OverlayMatData$Chromosome==1,"Stop"])         
  ### Init vector with positions for x-axis labeling                                 
  LabelPos = Len/2
  Lentmp = Len
  chromosomes = Chromosomes[Chromosomes!=1]         
  for(chromosome in chromosomes){
    Lentmp = Lentmp + max(OverlayMatData[OverlayMatData$Chromosome==chromosome,"Stop"])
    OverlayMatData[OverlayMatData$Chromosome==chromosome,"Start"] = OverlayMatData[OverlayMatData$Chromosome==chromosome,"Start"]+max(Len)
    OverlayMatData[OverlayMatData$Chromosome==chromosome,"Stop"] = OverlayMatData[OverlayMatData$Chromosome==chromosome,"Stop"]+max(Len)
    labelpos = (max(OverlayMatData[OverlayMatData$Chromosome==chromosome,"Stop"])/2) + (max(Len)/2)
    LabelPos = c(LabelPos,labelpos)
    Len = c(Len,Lentmp)
  }
  Len <<- Len
...
RunOverlayAnalysis = function(Samples,Paths="",Save_path="",species="Mouse",Method="Copywriter",resolution=20000,AberrationCutoff=0.2,SummaryStat="Proportion",ChromomsomesToRemove=c("X","Y"),Ylim="5",format="pdf",Suffix="")                         {
  ...
  fillOverlayMat(Samples,Paths=Paths,AberrationCutoff=AberrationCutoff)
  
  # NEW NIKLAS 01.12.2021
  if(F){
    
    # this keyword will be used to combine column (e.g. combine multiple Met or PPT columns for each mouse)
    keyword <- "Met"
    keyword <- "PPT"
    Suffix <- paste0(Suffix, ".collapsed", keyword)
    
    overlayDT <- data.table(OverlayMat)
    subtargets <- targets[FileID %in% Samples] # get subset of annotation
    ...
    for(mouseID in mouseIDs){
      ...
      setnames(overlayDT, "NEW", paste0(mouseID, "_", keyword, "Gain"))
      ...
      # add new column and delete the other ones
      overlayDT[, NEW := newCol]
      setnames(overlayDT, "NEW", paste0(mouseID, "_", keyword, "Del"))
      overlayDT[, (sampleColnames) := NULL]
      
      OverlayMat <- data.frame(overlayDT)
  ...
  combineOverlayMat(SummaryStat=SummaryStat)
  ReformatOverlayMat()
