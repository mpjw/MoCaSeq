...
# working_directory=/media/rad/HDD1/hPDAC_
...
#bash /opt/MoCaSeq/repository/vcf_annotation/Wrapper_helper_for_Niklas_convertToMoCaSeq.sh hPDAC03_HD_LivMet-1
...
probesbed=$genome_dir"/GRCh38.SureSelect_Human_All_Exon_V5_hg38.bed"

#first delete existing data
rm -f ${name}/results/rescued/${name}_Mutect2_combined.tsv ${name}/results/VariantAnnotation/*
...
mkdir  -p ${name}/results/VariantAnnotation
chown -R 1000:1000 ${name}/results/VariantAnnotation

# copy result files
echo "Copy result files"
cp ${annTmp}/finalAnnotation/${name}*  ${name}/results/VariantAnnotation/
