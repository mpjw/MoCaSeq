...
# DEBUG
# setwd("/run/user/1000/gvfs/sftp:host=172.21.251.53,user=rad/media/rad/HDD1/hPDAC_Cohort_Analysis/")
...
GenesOutFile <- paste0(name,".Tumor.center-mode.genes.tsv")
...
# ANNOTATE GENES AND PRINT SEGMENTS FILE
genecode_file = "../hsa_GRCh38_gencode_v36_genes.tsv"
genesDT = fread(genecode_file)
genesGR <- makeGRangesFromDataFrame(genesDT, keep.extra.columns = T)
segDT2 <- plotsegDT[, .(chr=chromosome, start, end, width, log2, ci_hi, ci_lo, depth, probes,weight)]
segGR2 <- makeGRangesFromDataFrame(segDT2, keep.extra.columns = T)
hits <- findOverlaps(segGR2, genesGR)
tmp <- cbind(segDT2[queryHits(hits)], genesDT[subjectHits(hits), .(geneName, geneID, geneBiotype)])
fwrite(tmp, GenesOutFile, sep="\t")



# SMOOTHING (merging segments in close proximity and similar mean value)
#DEBUG <- copy(plotsegDT)

# merge similar neighboring segments  
MinDifToMerge <- 0.01
setorder(plotsegDT, chromosome, start)
plotsegDT[, DifNextSeg := abs(plotValue - data.table::shift(plotValue)), by = chromosome]
plotsegDT[, DistNextSeg := abs(start - data.table::shift(end)), by = chromosome]
plotsegDT[, mergeID := as.numeric(.I)]
plotsegDT[DifNextSeg < MinDifToMerge & DistNextSeg < 10000000, mergeID := NA]
plotsegDT[, mergeID := na.locf(mergeID)]

plotsegDT <- unique(plotsegDT[, .(chromosome, segID=as.integer(mean(ID)), 
                              start=min(start), end=max(end), plotValue=mean(plotValue)), by=mergeID])
plotsegDT[, mergeID := NULL]
