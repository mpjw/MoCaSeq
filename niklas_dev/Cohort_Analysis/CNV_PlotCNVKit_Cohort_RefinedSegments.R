...
#SegmentsFile = paste(name,".Tumor.center-biweight.cns",sep="")
#CountsFile = paste(name,".Tumor.center-biweight.cnr",sep="")
...
# # remove centromeres and alternative haplotype regions from the data
# load("/media/rad/HDD1/Lookups/hg38_cents-haplo-mappability.RData")
# 
# hits <- findOverlaps(segbinGR,cents.gr)
# segbinDT[unique(queryHits(hits)), hitCent := "yes"]
# 
# hits <- findOverlaps(segbinGR,haplo.gr)
# segbinDT[unique(queryHits(hits)), hitHaploAlt := "yes"]
# 
# hits <- findOverlaps(segbinGR, germCNV.gr)
# segbinDT[unique(queryHits(hits)), hitClinVarCNV := "yes"]
# 
# hits <- findOverlaps(segbinGR,custom.gr)
# segbinDT[unique(queryHits(hits)), hitCustom := "yes"]
# 
# # input: x[, .(chr=Chromosome, start=genomicStart,end=genomicStop, ID)])
# mapvals <- GetMappability(segbinDT)
# mapvals[is.na(mappAbility), mappAbility := 0]
# segbinDT <- merge(segbinDT, mapvals, all.x=T, sort=F, by="ID")
# 
# # choose NOISE settings
# mappabilityCutoff <- 0.5
# segbinDT[!is.na(hitCent) | !is.na(hitHaploAlt) | !is.na(hitCustom) | mappAbility < mappabilityCutoff , noise := "NOISE"]
# 
# 
